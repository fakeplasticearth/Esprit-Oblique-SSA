...
Составим для первого примера ряд, где в тренде есть только сигнальный корень кратности 2. Построим тренд с помощью корня $\mu_1 = 0.47 - 0.86i$ кратности $m_1 = 2$. Также добавим гармоники $0.6 \mathsf{cos}(\frac{2\pi n}{4})$ и $0.6 \mathsf{cos}(\frac{2\pi n}{6} - \frac{\pi}{3})$. 

Проверим насколько оправданны такие усложнения модели.
```{r}
  mu1 = 0.47 + 0.86i
  print(paste0("mu_1: ", mu1, ", Mod: ", Mod(mu1)))
  ...
  eossa_old <- eossa(s, nested.groups = list(1:4), k = 2)
  res_old <- reconstruct(eossa_old, groups = eossa_old$iossa.groups)
```

Посмотрим на ошибку для обычного EOSSA.
```{r}
  print("Сумма модулей ошибок, первая выделенная компонента.")
  for (i in 1:2){
    print(paste0("Сумма модулей ошибок, сравнение с компонентой номер ", i, ": ", sum(abs(res_old$F1 - comps[[i]]))))
  }

  print("Сумма модулей ошибок, вторая выделенная компонента.")
  for (i in 1:2){
    print(paste0("Сумма модулей ошибок, сравнение с компонентой номер ", i, ": ", sum(abs(res_old$F2 - comps[[i]]))))
  }
```

Как можно увидеть, EOSSA для простых корней не выделил компоненты корректно.
...
Составим для первого примера ряд, где в модели тренда сигнальные корни большой кратности. Построим тренд с помощью корня $\mu_1 = 0.32 - 0.94i$ кратности $m_1 = 4$ и корня $\mu_2 = 0.26 + 0.96i$ кратности $m_2 = 3$. Также добавим гармоники $0.4 \mathsf{cos}(\frac{2\pi n}{12})$ и $0.4 \mathsf{cos}(\frac{2\pi n}{6} - \frac{\pi}{4})$. 
...
```{r}
  ...
  comp1 <- function(j) {
    return((5.3 - 0.06 * j + 3e-4 * j ^ 2 - 7.4e-6 * j ^ 3) * mu1 ^ j)
  }
  
  comp2 <- function(j) {
    return((-2.3 + 0.02 * j - 3.45e-4 * j ^ 2) * mu2 ^ j)
  }
  ...
  eoss <- eossa(s, nested.groups = list(1:11), k = 4)
  res_old = reconstruct(eoss, groups = eoss$iossa.groups)
  
  res_exp = eossa_exp(s, 11, cl_eps = 1e-2)
```
...
По этим результатам можно понять, что обычный EOSSA неправильно выделил компоненты тренда, неправильно выделил гармоники.
...
```{r}
  ...
  print(paste0("Сумма модулей ошибок, идентификация третьей компоненты: ", sum(abs(res_exp[[1]] + res_exp[[6]] - comps[[3]]))))
  print(paste0("Сумма модулей ошибок, идентификация четвертой компоненты: ", sum(abs(res_exp[[4]] + res_exp[[5]] - comps[[4]]))))
```
...
Полученные результаты показали, что обычный EOSSA не идентифицирует компоненты точно, если в модели ряда присутствуют сигнальные корни кратности два или больше. При этом обобщенный EOSSA выделяет компоненты в таких случаях точно (если кратность корней небольшая).